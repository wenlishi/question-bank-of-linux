...
# 3. 执行打包命令
pyinstaller --onefile --windowed --name="题库刷题软件" --add-data="data;data" --add-data="ui;ui" --add-data="core;core" main.py
```
...
打包完成后，生成的文件位于：
- `user_version/dist/题库刷题软件.exe` - 主程序文件
- `user_version/build/` - 临时构建文件（可删除）
- `user_version/题库刷题软件.spec` - PyInstaller配置文件（可删除）
